...
120 columns rather than the LLVM default of 80. ML and graph code tends to have
longer identifiers and nested template types; 120 gives enough room without
forcing awkward wraps.
...
When a function call or declaration doesn't fit on one line, every argument/parameter
gets its own line. Mixed "bin-packing" (some on one line, some wrapped) is
not allowed.
...
`template <...>` always appears on its own line, keeping the declaration
signature visually separate from the template header.
...
Raw string literals with the `pb` delimiter (e.g. `R"pb(...)pb"`) are
formatted as TextProto using Google style, matching the protobuf idiom used
...
A broad set of check families is enabled to catch bugs, enforce modern C++
idioms, and maintain readability. All warnings are errors — there is no
...
Some checks in the above families are disabled where they produce excessive
noise or conflict with common patterns in this codebase:
