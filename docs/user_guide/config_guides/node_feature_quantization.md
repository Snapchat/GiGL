...
Node feature quantization is an opt-in preprocessing setting that stores selected scalar node features in packed low-bit
form. GiGL automatically reconstructs approximate floating-point values before a sampled batch reaches the model.

Use it when node features are a meaningful memory or data-transfer cost and a small accuracy tradeoff is acceptable.
Start with a baseline run, then evaluate quantization on your task metric.
...
In the existing `NodeDataPreprocessingSpec` for each node type to quantize, add this field:
...
The keys must be distinct scalar fields already listed in `features_outputs`. GiGL supports `1`, `2`, `4`, and `8` bits.
...
Packing selected node features can reduce their stored and transferred payload. GiGL dequantizes sampled features at
runtime, so the model receives approximate `float` values rather than the original values.
...
Choose only finite scalar feature fields from `features_outputs`. Every key must exist there and be unique. A scalar
field has one number per node, such as `age=42`. A vector-valued field has a list or array per node, such as
...
No. GiGL saves quantization metadata during preprocessing and dequantizes sampled node features before they are passed
to the model.

### Can I quantize only some node features?
...
Quantization reduces the selected node-feature payload, not all training work. If data loading, feature transfer, or
memory capacity is not the bottleneck, end-to-end training time may not improve. Compare loader time, accelerator
utilization, and memory usage with the unquantized baseline before increasing the quantization level. Inference may see
more upside when it is feature-transfer or memory-bound; measure it separately from training.

### Does this support edge features?

No. This setting currently supports node features only.
