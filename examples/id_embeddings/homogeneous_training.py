...
WORLD_SIZE=1 RANK=0 MASTER_ADDR="localhost" MASTER_PORT=20000 python -m examples.id_embeddings.homogeneous_training --task_config_uri=<frozen_task_config_uri>
...
def _compute_bpr_batch(
    ...
    if rand_emb.size(0) < total_needed:
        ...
        rand_pool = rand_emb[:total_needed]
...
def _training_process(
    ...
    logger.info(f"---Rank {rank} initializing optimizer")
    optimizer = AdamW(
...
def _run_example_training(
    ...
    all_ids = to_homogeneous(dataset.node_ids)  # Tensor of global node IDs
    max_id = int(all_ids.max().item())
    num_nodes_total = max_id + 1
