...
def _inference_process(
    ...
    logger.info(
        f"--- Machine {machine_rank} local rank {local_rank} finished inference for node type {inference_node_type}."
    )
    ...
    logger.info(
        f"--- Machine {machine_rank} local rank {local_rank} finished writing embeddings to GCS for node type {inference_node_type}, which took {time.time()-write_embedding_start_time:.2f} seconds"
    )
