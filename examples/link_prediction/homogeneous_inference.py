...
def _inference_process(
    ...
    for batch_idx, data in enumerate(data_loader):
        ...
        if batch_idx > 0 and batch_idx % log_every_n_batch == 0:
            logger.info(
                f"rank {torch.distributed.get_rank()} processed {batch_idx} batches. "
    ...
    logger.info(f"--- Rank {torch.distributed.get_rank()} finished inference.")
    ...
    logger.info(
        f"--- Rank {torch.distributed.get_rank()} finished writing embeddings to GCS, which took {time.time()-write_embedding_start_time:.2f} seconds"
    )
