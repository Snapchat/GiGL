...
object TaskOutputValidator {
  ...
  def validationHelper(
    ...
    val setOfFeauturlessNeighborhoodNodes: Set[Node] = neighborhoodNodes
    ...
    edgesToValidate
      .foreach(edge => {
        ...
        featurelessNodesInEdge.foreach(nodePb => {
          if (!setOfFeauturlessNeighborhoodNodes.contains(nodePb)) {
            throw new RuntimeException(
              s"Output Validation failed: node ${nodePb} of Edge ${edge} not present in neighborhood graph. With neighborhood nodes: ${setOfFeauturlessNeighborhoodNodes}",
            )
          }
        })
      })
