...
class TestGetIdsOnRank(unittest.TestCase):
    def test_tensor_partition_book(self):
        ...
        self.assertTrue(
            torch.equal(get_ids_on_rank(partition_book, 0), torch.tensor([0, 2]))
        )
        self.assertTrue(
            torch.equal(get_ids_on_rank(partition_book, 1), torch.tensor([1, 3]))
        )
    ...
    def test_range_partition_book(self):
        ...
        self.assertTrue(torch.equal(get_ids_on_rank(range_pb, 0), torch.arange(0, 5)))
        self.assertTrue(torch.equal(get_ids_on_rank(range_pb, 1), torch.arange(5, 10)))
...
class TestBuildBalancedRangePartitionBook(unittest.TestCase):
    def test_divides_evenly(self):
        ...
        self.assertEqual(pb.partition_bounds.tolist(), [5, 10])
    ...
    def test_divides_unevenly(self):
        ...
        self.assertEqual(pb.partition_bounds.tolist(), [3, 5, 7])
