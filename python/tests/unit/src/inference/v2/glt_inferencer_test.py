...
class GLTInferencerTest(unittest.TestCase):
    """Test cases for GLTInferencer region override functionality."""
    ...
    @patch(
        "gigl.src.inference.v2.glt_inferencer.GbmlConfigPbWrapper.get_gbml_config_pb_wrapper_from_uri"
    )
    @patch("gigl.src.inference.v2.glt_inferencer.VertexAIService")
    def test_region_override_is_applied_when_set(
        ...
        mock_inferencer_config = VertexAiResourceConfig()
        mock_inferencer_config.gcp_region_override = self.test_override_region
        mock_inferencer_config.machine_type = "n1-standard-4"
        mock_inferencer_config.gpu_type = ""  # CPU inference
        mock_inferencer_config.num_replicas = 1
        mock_resource_config_wrapper.inferencer_config = mock_inferencer_config

        mock_get_resource_config.return_value = mock_resource_config_wrapper

        # Mock GBML config
        mock_gbml_config_wrapper = MagicMock()
        mock_gbml_config_wrapper.inferencer_config.command = "python test.py"
        mock_gbml_config_wrapper.inferencer_config.inferencer_args = {}
    ...
    def test_region_fallback_when_no_override(
        ...
        mock_inferencer_config = VertexAiResourceConfig()
        mock_inferencer_config.gcp_region_override = ""  # No override
        mock_inferencer_config.machine_type = "n1-standard-4"
        mock_inferencer_config.gpu_type = ""  # CPU inference
        mock_inferencer_config.num_replicas = 1
        mock_resource_config_wrapper.inferencer_config = mock_inferencer_config
