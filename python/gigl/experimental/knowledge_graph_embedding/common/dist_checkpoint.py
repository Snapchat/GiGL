...
class AppState(Stateful):
    ...
    call state_dict/loade_stat_dict as needed in the dcp.save/load APIs.
...
#     dcp_fn = dcp.async_save if should_save_asynchronously else dcp.save
#     dcp_fn(state_dict, storage_writer=writer)
#     if isinstance(checkpoint_id, GcsUri):
...
#     """
#     logger.info(f"inside save_checkpoint_to_uri with condition: {should_save_asynchronously}")
#     def _save_checkpoint(
#         state_dict: STATE_DICT_TYPE, checkpoint_id: Uri, should_save_asynchronously: bool = False
#     ) -> Uri:
#         assert isinstance(checkpoint_id, LocalUri) or isinstance(
#             checkpoint_id, GcsUri
#         ), "checkpoint_id must be a LocalUri or GcsUri."
...
#             checkpoint_future.result()  # Wait for the async save to complete
...
#         # Use a ThreadPoolExecutor to run the save operation asynchronously
#         # This allows the main thread to continue while the checkpoint is being saved.
#         # The Future object will be returned, which can be used to check the status of the
#         # save operation or to wait for it to complete.
#         future = executor.submit(
#             _save_checkpoint, state_dict, checkpoint_id, should_save_asynchronously
#         )
#         return future
#     else:
#         logger.info(f"Saving checkpoint synchronously to {checkpoint_id}")
#         return _save_checkpoint(state_dict, checkpoint_id, should_save_asynchronously)
