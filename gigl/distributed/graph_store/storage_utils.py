...
requests from compute nodes.  Each storage node initialises a GLT
...
per-server lock when initialising samplers.  If connections from multiple
...
Storage nodes wait for all compute processes to connect, then serve
sampling requests until the compute processes signal shutdown via
