...
class DistServer:
    ...
    def get_ablp_input(
        ...
        Raises:
            ValueError: If the split or supervision schema is invalid.
        """
        if not request.supervision_edge_types:
            raise ValueError("supervision_edge_types must be non-empty.")
        if len(set(request.supervision_edge_types)) != len(
            request.supervision_edge_types
        ):
            raise ValueError(
                "supervision_edge_types must not contain duplicates, received "
                f"{request.supervision_edge_types}."
            )

        anchor_endpoint_index = 0 if self.dataset.edge_dir == "out" else 2
        invalid_supervision_edge_types = [
            edge_type
            for edge_type in request.supervision_edge_types
            if edge_type[anchor_endpoint_index] != request.node_type
        ]
        if invalid_supervision_edge_types:
            raise ValueError(
                f"For edge_dir={self.dataset.edge_dir!r}, registered supervision "
                f"edge types must have anchor node type {request.node_type!r} at "
                f"index {anchor_endpoint_index}; received invalid edge types "
        ...
        for supervision_edge_type in request.supervision_edge_types:
            (
                positive_label_edge_type,
                negative_label_edge_type,
            ) = select_label_edge_types(supervision_edge_type, edge_types)
            labels[supervision_edge_type] = get_labels_for_anchor_nodes(
                self.dataset,
                anchors,
                positive_label_edge_type,
                negative_label_edge_type,
