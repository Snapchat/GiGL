...
class HGTConv(MessagePassing):
    ...
    def _construct_src_node_feat(
        ...
        ks_cat = torch.cat(ks, dim=0).transpose(0, 1).reshape(-1, D)
        vs_cat = torch.cat(vs, dim=0).transpose(0, 1).reshape(-1, D)
        type_vec = torch.cat(type_list, dim=1).flatten()

        k = self.k_rel(ks_cat, type_vec).view(H, -1, D).transpose(0, 1)
        v = self.v_rel(vs_cat, type_vec).view(H, -1, D).transpose(0, 1)
