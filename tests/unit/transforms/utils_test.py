...
from gigl.transforms.utils import add_edge_attr, add_node_attr
...
class TestAddEdgeAttr(TestCase):
    def test_add_edge_attr_with_attr_name(self):
        ...
        self.assert_tensor_equality(
            data["item", "bought_by", "user"].test_attr, values[3:]
        )
    ...
    def test_add_edge_attr_with_dict(self):
        ...
        self.assertEqual(data["item", "bought_by", "user"].test_attr.shape, (3, 8))
        self.assert_tensor_equality(
            data["user", "buys", "item"].test_attr, values[("user", "buys", "item")]
        )
        self.assert_tensor_equality(
            data["item", "bought_by", "user"].test_attr,
            values[("item", "bought_by", "user")],
        )
    ...
    def test_add_edge_attr_concatenate_to_edge_attr(self):
        ...
        self.assertEqual(
            data["item", "bought_by", "user"].edge_attr.shape, (3, 12)
        )  # 4 + 8

        # Check original features are preserved
        self.assert_tensor_equality(
            data["user", "buys", "item"].edge_attr[:, :4], original_buys_attr
        )
        self.assert_tensor_equality(
            data["item", "bought_by", "user"].edge_attr[:, :4], original_bought_by_attr
        )
