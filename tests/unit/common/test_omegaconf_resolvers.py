...
class TestGiglResolver(TestCase):
    ...
    The resolver pulls runtime values (set by ``launch_custom`` before
    re-resolving the proto's ``command`` / ``args`` strings) from a
    module-level dict, falling back to the literal placeholder
    ``"${gigl:<key>}"`` so the first-pass YAML load is lossless.
    """
    ...
    def test_gigl_resolver_round_trips_unset_keys_through_yaml(self):
        ...
        # placeholder must survive a YAML→OmegaConf round trip so
        # launch_custom can re-resolve it later with values set.
        yaml_config = """
        custom:
